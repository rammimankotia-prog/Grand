...
# Regex to replace the car itinerary
pattern = re.compile(r'itinerary:\s*\[.*?\]\,', re.DOTALL)
# Only replace the first occurrence (which is the car itinerary)
js_content = pattern.sub(new_itinerary + ',', js_content, count=1)

with open(js_file, 'w', encoding='utf-8') as f:
    f.write(js_content)
    
print("Successfully updated 8-day car itinerary in JS")
